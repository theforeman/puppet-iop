...
describe 'iop::metadata_downloader' do
  before(:all) do
    ...
    on default, 'rm -rf /var/www/html/pub/https*'
  ...
  context 'with default parameters' do
    ...
    describe file('/etc/systemd/system/iop-metadata-download-cvemap.service') do
      ...
      its(:content) { is_expected.to match %r{/usr/bin/curl} }
      its(:content) { is_expected.to match %r{--create-dirs --fail --silent --remote-time --time-cond} }
    ...
    context 'manual timer execution' do
      ...
      describe file('/var/www/html/pub/https:/security.access.redhat.com/data/meta/v1/cvemap.xml') do
        it { is_expected.to be_file }
      ...
      describe command('head -5 /var/www/html/pub/https:/security.access.redhat.com/data/meta/v1/cvemap.xml') do
