...
class iop::service_compliance (
    ...
    settings     => {
      ...
      'Container' => {
        ...
        'Environment'   => [
          ...
          'SETTINGS__REPORT_DOWNLOAD_SSL_ONLY=true',
          'MAX_INIT_TIMEOUT_SECONDS=120',
          'SETTINGS__REDIS__SSL=true',
        ],
        'Secret'        => [
          "${database_username_secret_name},type=env,target=POSTGRES_USER",
          "${database_password_secret_name},type=env,target=POSTGRES_PASSWORD",
  ...
  podman::quadlet { 'iop-service-compl-ssg':
    ...
    settings     => {
      ...
      'Container' => {
        'Image'         => $SSG_image,
        'ContainerName' => 'iop-service-compl-ssg',
        'Network'       => 'iop-core-network',
        'Volume'        => $socket_volume,
      },
      'Service'   => {
  ...
  podman::quadlet { 'iop-service-compl-reindex-db':
    ensure         => $ensure,
    quadlet_type   => 'container',
    user           => 'root',
    service_ensure => 'stopped',
    ...
    settings       => {
      'Unit'      => {
        'Description' => 'Compliance Reindex DB Job',
        'Wants'       => ['iop-service-compl-dbmigrate.service'],
        'After'       => ['iop-service-compl-dbmigrate.service'],
      },
      'Container' => {
        'Image'         => $image,
        'ContainerName' => 'iop-service-compl-reindex-db',
        'Network'       => 'iop-core-network',
        'Volume'        => $socket_volume,
        'Environment'   => [
          'APPLICATION_TYPE=compliance-reindex-db',
          'RAILS_ENV=foreman',
          'RAILS_LOG_TO_STDOUT=true',
          'PATH_PREFIX=/api',
          'APP_NAME=compliance-backend',
  ...
  systemd::timer { 'iop-service-compl-reindex-db.timer':
    ensure        => $ensure,
    enable        => $timer_ensure,
    active        => $timer_ensure,
    service_unit  => 'iop-service-compl-reindex-db.service',
    timer_content => file('iop/iop-service-compl-reindex-db.timer'),
    require       => Podman::Quadlet['iop-service-compl-reindex-db'],
  }
}
